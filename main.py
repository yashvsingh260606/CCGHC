...
MONGO_URL = "mongodb+srv://luckystoreagra:Agra%402023@cluster0.kvnwnyh.mongodb.net/?retryWrites=true&w=majority"
# Replace with your MongoDB URI
# --- Configuration ---

# List of Telegram user IDs who are bot admins
# Replace with your own Telegram user IDs
BOT_ADMINS = [7361215114, 6891578700, 7544534283]  # <--- Replace these with your Telegram user IDs
# --- MongoDB Setup ---
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name = USERS.get(user.id, {}).get('name', 'Player')

    await update.message.reply_text(
        f"Welcome to HandCricket, {name}!\nUse /register to get 4000 🪙 coins."
    )
...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if USERS.get(user.id, {}).get("registered", False):
        await update.message.reply_text("✅ You are already registered and have claimed your coins!")
        return

    # Mark user as registered and give coins once
    USERS[user.id]["registered"] = True
    USERS[user.id]["coins"] = 4000

    await update.message.reply_text("🎉 Registration successful! You now have 4000 🪙 coins.")
...
async def profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    profile_text = (
        f"{user_data.get('name', 'Player')}'s Profile\n\n"
        f"Name: {user_data.get('name', 'Player')}\n"
        f"ID: {user.id}\n"
...
def main():
    keep_alive()  # 👈 MUST be called before anything else
    ...
def main():
